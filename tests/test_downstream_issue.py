...
class TestDownstreamIssue(unittest.TestCase):
    ...
    @mock.patch("jira.client.JIRA")
    @mock.patch(PATH + "check_jira_status")
    def test_close_duplicates_no_matching(
        self,
        mock_check_jira_status,
        mock_client,
        ...
        # Call the function
        response = d.close_duplicates(issue=self.mock_issue, config=self.mock_config)

        # Assert everything was called correctly
        mock_get_jira_client.assert_called_with(self.mock_issue, self.mock_config)
        mock_matching_jira_issue_query.assert_called_with(
            mock_client, self.mock_issue, self.mock_config, free=True
        )
    ...
    def test_close_duplicates(
        ...
        mock_close_as_duplicate.assert_called_with(
            mock_client, mock_item, mock_item, self.mock_config
        )
        self.assertEqual(None, response)

    @mock.patch("jira.client.JIRA")
    def test_close_as_duplicate_errors(self, mock_client):
        """
        ...
        mock_client.add_comment.assert_any_call(
            mock_duplicate, "Marking as duplicate of mock_key"
        )
        ...
        mock_client.transition_issue.assert_any_call(mock_duplicate, "1234")

    @mock.patch("jira.client.JIRA")
    def test_close_as_duplicate(self, mock_client):
        """
